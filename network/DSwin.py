...
from timm.models.layers import DropPath, to_2tuple, trunc_normal_, to_ntuple, _assert, PatchEmbed
from timm.models.helpers import named_apply
from timm.models.vision_transformer import get_init_weights_vit

from modules.utils import batch_index_fill, batch_index_select
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows, v, attn = self.attn(x_windows, mask=self.attn_mask)  # num_win*B, window_size*window_size, C
...
class AdaSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # W-MSA/SW-MSA
        attn_windows, v, attn = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., norm_layer=nn.LayerNorm, downsample=None, keep_rate=1.0, pruning_num=0):
        ...
        #assert len(keep_rate) == len(pruning_loc), "len(keep_rate) != len(pruning_loc)"
        ...
        if keep_rate == 1.0:
            ...
            self.blocks = nn.ModuleList([
                SwinTransformerBlock(
                    ...
                    mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop,
                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer=norm_layer)
                for i in range(pruning_num)] + [
                AdaSwinTransformerBlock(
                    dim=dim, input_resolution=input_resolution, num_heads=num_heads, head_dim=head_dim,
                    window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2,
                    mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop,
                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer=norm_layer)
                for i in range(pruning_num, depth)])
    ...
    def forward(self, x):
        ...
        if self.keep_rate == 1.0:
            ...
            for blk_idx, blk in enumerate(self.blocks):
                if blk_idx == self.pruning_num and self.keep_rate<1:
                    x, mask = self.score_predictor(x, mask, self.keep_rate)
                    decisions = mask
                if blk_idx < self.pruning_num:
                    x, v, attn = blk(x)
                else:
                    x, v, attn = blk(x, mask)
                final_attn = attn if final_attn is None else attn + final_attn
        ...
        if self.downsample is not None:
            v = x
            x = self.downsample(x)
        return x, decisions, v, final_attn
...
class AdaSwinTransformer(nn.Module):
    ...
    def __init__(
            ...
            keep_rate=[0.9,0.7,0.5], pruning_loc=[4,8,12], **kwargs):
        super().__init__()
        assert global_pool in ('', 'avg')
        self.num_classes = num_classes
        self.global_pool = global_pool
        self.num_layers = len(depths)  # depths: [2, 2, 18, 2]
        ...
        head_dim = to_ntuple(self.num_layers)(head_dim)  # None
        ...
        for i in range(self.num_layers):
            layers.append(BasicLayer(
                ...
                keep_rate=keep_rate[i], pruning_num=pruning_loc[i]
            ))
    ...
    def forward_features(self, x):
        ...
        for i_layer, layer in enumerate(self.layers):
            if i_layer == 2:
                L = x.shape[1]
            x, decisions, v, attn = layer(x)  # NOTE
            if decisions is not None:
                decision_mask_list.append(decisions)
            else:
                decision_mask_list.append(None)
            feature_list.append(x)
            # NOTE
            attn = torch.mean(attn, dim=1)  # average at multi-head dimension B_,H,N,N --> B_,N,N
            sizeW = int(math.sqrt(attn.shape[0]//B))
            attn = attn.view(B, sizeW, sizeW, N, N)
            attn = attn.permute(0, 1, 3, 2, 4)
            attn = attn.reshape(B, sizeW*N, sizeW*N)
        ...
        for idx, attn in enumerate(attn_list):
            attn = F.interpolate(attn.unsqueeze(1), (target_size, target_size)).squeeze(1)
            final_attn = weights[idx]*attn if final_attn is None else (weights[idx]*attn + final_attn)
        final_attn = torch.mean(final_attn, dim=1).unsqueeze(-1)  # B,N,N --> B,N,1
        ...
        if self.training:
            for item in decision_mask_list:
                if item is not None:
                    # print(decision_mask.device, item.device)
                    decision_mask *= item.squeeze(-1)
        else:
            # long_mask = None
            for i in range(len(decision_mask_list)):
                if decision_mask_list[i] is None:
                    continue
                long_mask = decision_mask_list[i][0]
                short_mask = decision_mask_list[i][1]
                temp = torch.ones(B, L, device=x.device)
                offset = torch.arange(B, dtype=torch.long, device=x.device).reshape(B, 1) * L
        ...
        # NOTE
        return x, patch_x, final_attn, decision_mask.unsqueeze(-1).detach(), decision_mask_list, feature_list
